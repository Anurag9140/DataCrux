...
def train_text_ml_models(X_train, X_test, y_train, y_test):
    ...
    X_train_vec = vectorizer.fit_transform(X_train.iloc[:, 0])  # Assuming first column is text
    ...
    return models, top_3_models, model_scores, top_5_models, vectorizer
...
def train_text_dl_models(X_train, X_test, y_train, y_test):
    # Load BERT tokenizer and model
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    bert_model = BertModel.from_pretrained('bert-base-uncased')

    # Get BERT embeddings
    X_train_embeddings = get_bert_embeddings(X_train.iloc[:, 0], tokenizer, bert_model)
    X_test_embeddings = get_bert_embeddings(X_test.iloc[:, 0], tokenizer, bert_model)

    # Train a simple classifier (e.g., Logistic Regression) on BERT embeddings
    model = LogisticRegression()
    model.fit(X_train_embeddings, y_train)
    y_pred = model.predict(X_test_embeddings)
    acc = accuracy_score(y_test, y_pred)
    print(f"\n🔹 **BERT Model Accuracy:** {acc:.4f}")
...
# Example usage
autoforge("/Users/anuragverma/Desktop/DataCrux/Iris.csv", data_type='csv',category='text')